...
const newTask = ref("");
const removeToDo = (selectedNo) => {
  let delIdx = todos.findIndex((todo) => todo.no == selectedNo);
  todos.splice(delIdx, 1);
};
const completed = (selectedNo) => {
  for (let i = 0; i < todos.length; i++) {
    if (todos[i].no == selectedNo) {
...
<template>
  ...
  <ul id="myUL">
    <li
      ...
      v-on:click.self="completed(todo.no)"
    >
      {{ todo.task }}
      <span class="close" v-on:click.stop="removeToDo(todo.no)">x</span>
    </li>
  </ul>
</template>
